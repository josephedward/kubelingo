...
def test_build_interactive_menu_shows_question_counts(mock_get_flagged, mock_yaml_loader):
    ...
    mock_loader_instance.discover.return_value = ["/path/to/kubectl_basics_quiz.yaml"]
    ...
    test_quiz_choice = next((c for c in choices if c['value'] == "/path/to/kubectl_basics_quiz.yaml"), None)

    assert test_quiz_choice is not None
    assert test_quiz_choice['name'] == "Kubectl Basics Quiz (10 questions)"

    # Verify that the loader's methods were called correctly
    mock_loader_instance.discover.assert_called_once()
    mock_loader_instance.load_file.assert_called_once_with("/path/to/kubectl_basics_quiz.yaml")
